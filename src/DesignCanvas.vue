...
function upUI(x:number, y:number){
    ...
    ctx?.drawImage(item.img, x,y, item.width, item.height, item.dx||0, item.dy||0, item.width, item.height, )
    item.x = x
    item.y = y
}
    ...
    switch (evt.type) {
        case 'mousedown':
            ...
            itemList.forEach((e, i)=>{
                if (!i || !e) return
                if (evt.offsetX>=e.dx && evt.offsetX<=e.dx+e.width && evt.offsetY>=e.dy && evt.offsetY<=e.dy+e.height){
                    selIdx = i
                }
            })
            if (!itemList[selIdx]) return
            last = [itemList[selIdx].x-evt.x, itemList[selIdx].y-evt.offsetY]
