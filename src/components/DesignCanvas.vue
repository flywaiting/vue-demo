...
function upUI(x: number, y: number) {
    ...
    if (bg && bg.img){
        ...
        ctx?.drawImage(bg.img, sx, sy, item.width, item.height, item.dx??0, item.dy??0, item.width, item.height)
    }

    // 过滤出需要绘制的内容
    itemList.filter((e, i)=>e && ((selIdx==0)!=(i==selIdx))).forEach(e=>{
        e.img && ctx?.drawImage(
            e.img,
            e.x??0,
            e.y??0,
...
function moveHandle(evt: MouseEvent) {
    switch (evt.type) {
        case 'mousedown':
            selIdx = Math.max(0, itemList.findIndex((e, i)=>{
                return  i && e && evt.offsetX>=(e.dx??0) && evt.offsetX<=(e.dx??0)+e.width && evt.offsetY>=(e.dy??0) && evt.offsetY<=(e.dy??0)+e.height
            }));
